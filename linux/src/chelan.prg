...
WHILE OK
	...
	cDocnr := Space(9)
	MaBox( 13, 11, 15, 31 )
	@ 14, 12 Say  "Docto nº:" Get cDocnr Pict "@!" Valid Doccherr( @cDocnr )
	Read
	IF LastKey() = ESC
		IF nConta != 0
...
WHILE OK
	...
	cDocnr := Space(9)
	MaBox( 13, 11, 15, 31 )
	@ 14, 12 Say  "Doc Nº..:" Get cDocnr Pict "@!" Valid Doccherr( @cDocnr )
	Read
	IF LastKey() = ESC
		IF nConta != 0
...
IF !( DbSeek( Var ) )
	...
	DbGoTop()
	Escolhe( 03, 01, 22, "Codi + '║' + Titular", "CODI TITULAR DA CONTA", aRotina )
	Var := Codi
EndIF
   ...
   WHILE OK
		...
		WHILE OK
         cDocnr  := cData + "-" + StrZero( ++nReg, 2)
			...
			@ 11, 11 Say "Docto. Nº..:" Get cDocnr   Pict "@K!" Valid CheqDoc( cDocnr )
...
IF !( DbSeek( Var ) )
	DbGoTop()
	Chemov->(Escolhe( 03, 00, 22, "Docnr + '│' + Dtoc( Data ) + '│' + Codi + '│' + Hist", "DOC Nº    DATA     CODI HISTORICO DO MOVIMENTO"))
	Var := Chemov->Docnr
EndIF
Return( OK )
...
WHILE Cheque->(Eval( oBloco )) .AND. !Eof() .AND. Rel_Ok()
	...
	WHILE Eval( oBloco1 ) .AND. Rel_Ok()
		IF Col >= 57
			lNovoCodigo := FALSO
			Qout( Padr( "Pagina Nº " + StrZero( ++Pagina,3 ), ( Tam/2 ) ) + Padl( Time(), ( Tam/2 ) ))
			Qout( Date())
			Qout( Padc( XNOMEFIR, Tam ))
			Qout( Padc( SISTEM_NA5, Tam ))
			Qout( Padc( cTitulo, Tam ))
			Qout( Repl( SEP, Tam ))
			Qout( "EMISSAO  DT LCTO        DOC.Nº    HISTORICO DO LANCAMENTO                              DEBITO            CREDITO              SALDO")
			Qout( Repl( SEP, Tam ))
...
WHILE OK
   ...
   cCpfCgc  := Space(14)
	WHILE OK
      ...
      @ 12, 11 Say "Docto Nº....:" Get cDocnr   Pict "@K!" // Valid Ch_Doc_Pre( cDocnr )
      @ 13, 11 Say "Valor.......:" Get nVlr     Pict "@E 9,999,999,999.99" Valid CheqVlr( nVlr )
...
WHILE OK
	...
	Case nChoice = 3
		cDocnr := Space(9)
		MaBox( 16, 20, 18, 42 )
		@ 17, 21 Say  "Docto Nº.:" Get cDocnr Pict "@!" Valid Ch_Doc_Err( @cDocnr )
		Read
		IF LastKey() = ESC
			ResTela( cScreen )
			Loop
...
IF ( DbSeek( Var))
	ErrorBeep()
	Alerta("Erro: Documento Ja Registrado ou Incluido por outra Estaçao...")
	AreaAnt( Arq_Ant, Ind_Ant )
	Return( FALSO )
...
Order( CHEPRE_DOCNR_VCTO )
IF !( DbSeek( cDocnr ))
	Escolhe( 03, 01, 22, "Docnr + '║' + Hist", "DOCTO Nº  HISTORICO DO MOVIMENTO")
	cDocnr := Docnr
EndIF
AreaAnt( Arq_Ant, Ind_Ant )
Return( OK )
...
@ 15,12 Say  "Contrato Nº......:" Get cFatura Pict "@!" Valid AchaContrato( @cFatura )
...
For nA := 1 To nQuant
   ...
   Qout(     "Clausula Segunda: A MICROBRAS ira repor as faltas de aulas se porventura seja constatada a sua culpa, como falta de professor,")
	...
	Qout("")
	Qout( 	 "   DOCTO Nº  EMISSAO    VENCTO            VALOR")
	Qout( 	 "   --------------------------------------------")
...
IF ( Empty( cCurso ) )
	ErrorBeep()
	Alerta( "Erro: Código Curso Invalido..." )
	Return( FALSO )
EndIF
Area( "Cursos" )
Cursos->(Order( CURSOS_CODI ))
IF Cursos->(DbSeek( cCurso ))
	ErrorBeep()
	Alerta("Erro: Codigo curso já registrado..." )
	cCurso := StrZero( Val( cCurso )+1,4)
	AreaAnt( Arq_Ant, Ind_Ant )
	Return( FALSO )
EndIF
...
Cursos->(Order( CURSOS_CODI ))
IF Cursos->(!DbSeek( cCurso ))
	Escolhe( 03, 01, 22, "Curso + '║' + Obs", "CODIGO DESCRICAO DO CURSO", aRotina )
	cCurso := Cursos->Curso
	cObs	 := Cursos->Obs
	IF nRow != Nil
		Write( nRow, nCol, Obs )
...
Write( 05, 00, Repl( SEP, Tam ) )
Write( 06, 00, "DOCTO Nº   EMISSAO   VENCTO            VALOR  DT PGTO       VALOR PAGO")
Write( 07, 00, Repl( SEP, Tam ) )
Return
...
IF Cursado->(!DbSeek( cDocnr ))
	...
	Else
		...
		Cursado->(Order( CURSADO_FATURA ))
		Escolhe( 03, 01, 22, "Fatura + '▌' + Receber->Nome", "Nº CONTR  ONOME DO CLIENTE")
		Cursado->(DbClearRel())
	EndIF
...
Conta->(Order( CONTA_CODI ))
IF Conta->( !DbSeek( cCodi ))
	Escolhe( 03, 01, 22, "Codi + '▌' + Hist","CODI HISTORICO DA CONTA ", aRotina  )
	cCodi  := Conta->Codi
EndIF
IF nRow != Nil
...
WHILE OK
	...
	@ 13, 11 Say "Docto. Nº........:" Get cDocnr   Pict "@K!" Valid CheqDoc( cDocnr )
	@ 14, 11 Say "Historico........:" Get cHist    Pict "@K!"
...
cCabec	:= "CONTA EMISSAO VENCTO   DOCT Nº   HISTORICO DO LANCAMENTO                     VALOR DO CHEQUE    BANCO      PRACA"
...
WHILE OK
   ...
   Write( 23, 01, "Saldo...: " + Tran( nSaldo, "@ECX 99,999,999,999.99"))
	...
	oBloco := {|| Chemov->Codi = cCodi }
		  cTela	:= Mensagem("Please, Aguarde... Anexando Registro nº 0000000. ESC Cancela.", Cor())
	nConta := 0
