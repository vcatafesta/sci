...
METHOD CriaNtx()
	...
	if Used() // 13:56 25/04/2018
		...
		For nX := 1 To nLen
			...
			Write( Self:Row+nX, Self:Col+1, Self:aNome_Ntx[nX] + Repl(chr(196) /*─*/, 24 - Len( Self:aNome_Ntx[nX])))
			Write( Self:Row+nX, Self:Col+25, Chr(254))
...
METHOD def ProgNtx
	...
	@ Self:Row-4, Self:Col+1 Say chr(254) + chr(32) + LTrim(Str(nReg)) + " de " + LTrim(Str(nUltimo )) + " Registros"
	@ Self:Row-3, Self:Col+1 Say chr(254) + chr(32) + cComplete + "%"
	@ Self:Row-2, Self:Col+1 Say Replicate(chr(32), 100/2 ) Color "W+/r"
	@ Self:Row-2, Self:Col+1 Say Replicate(chr(254), nPorcento/2 ) Color "W+/r"
	return true
