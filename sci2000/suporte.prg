...
WHILE OK
	...
	cPraca := Space( 30 )
	cEsta  := Space( 02 )
	@ 16, 66 Get cCodi Pict PIC_RECEBER_CODI Valid Cliente( @cCodi, @cPraca,, @cEsta )
	Read
	...
	MaBox( 01, 00, 14, 79, " DOCTO N§     EMIS   VENCTO ATRA  NOMINAL     DESC    MULTA    JUROS    GERAL     " )
	WHILE OK
		nEscolha := Achoice( 03, 01, 13, 78, aDoc, alDisponivel,, nPosicao )
		IF nEscolha = 0
			Exit
		EndIf
		_QtDup++
...
oBrowse:HotKey( F4, {|| DuplicaLista( oBrowse ) })
...
For nField := 1 To nConta
	xAlias->(FieldPut( nField, Lista->(FieldGet( nField ))))
Next
IF Lista->(Incluiu())
	For nField := 1 To nConta
		Lista->(FieldPut( nField, xAlias->(FieldGet( nField ))))
	Next
	xRegLocal := Lista->(Recno())
	Lista->(Libera())
	Lista->(Order( LISTA_CODIGO ))
	Lista->(DbGoBottom())
...
oBrowse:Titulo   := "CONSULTA/ALTERACAO DE FORMAS DE PGTO"
