...
METHOD CriaNtx()
	...
	if Used() // 13:56 25/04/2018
		...
		For nX := 1 To nLen
			...
			Write( Self:Row+nX, Self:Col+1, Self:aNome_Ntx[nX] + Repl("─", 24 - Len( Self:aNome_Ntx[nX])))
...
METHOD def ProgNtx
	...
	@ Self:Row-4, Self:Col+1 Say "■ " + LTrim(Str(nReg)) + " de " + LTrim(Str(nUltimo )) + " Registros"
	@ Self:Row-3, Self:Col+1 Say "■ " + cComplete + "%"
	@ Self:Row-2, Self:Col+1 Say Replicate(" ", 100/2 ) Color "W+/r"
	@ Self:Row-2, Self:Col+1 Say Replicate("█", nPorcento/2 ) Color "W+/r"
	return true
