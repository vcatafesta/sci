...
While Eval( oBloco ) .AND. REL_OK()
	if Col >=  58
		...
		Write( 06, 00,"DATA     DOCTO Nº  DESCRICAO                                               VALOR")
...
if Empty( cDocnr )
	ErrorBeep()
	Alerta( "ERRO: Codigo Documento Inválido." )
	return( FALSO )
endif
if !DbSeek( cDocnr )
	 ErrorBeep()
...
WHILE !Eof() .AND. REL_OK()
	if Col >=  57
		...
		Write( 05, 00, Repl( SEP, Tam ) )
		Write( 06, 00, "NOME DO CLIENTE                           EMISSAO  VENCTO ATRAS  Nº DOCTO  FATURA   CP    VLR FATURA  PERC   COM. TOTAL COM.LIBERADA")
		Write( 07, 00, Repl( SEP, Tam ) )
		Col := 08
	endif
...
While Eval( oBloco ) .AND. Rel_Ok()
	if Col >=  58
		...
		Write( 05, 00, Linha5(Tam))
		Write( 06, 00,"CLIENTE/HISTORICO        DATA     VCTO ATR  FATURA    Nº DOCTO FP RG  PERC  VLR FATU  COMISSAO  COM.BLOQ  COM.DISP CREDITOS   ADIANT")
		Write( 07, 00, Repl( SEP, Tam ) )
		Col := 8
	endif
	if Eval( oSkipper )
...
EndDo
oMenu:Limpa()
Alerta('Tarefa efetuada. Se ainda houver problemas,;reconstrua o arquivo NOTA e faça novamente.')
...
WHILE OK
	...
	Case Choice = 1
		MaBox( 16, 10, 19, 76 )
		@ 17, 11 Say "Vendedor.........:" Get cCodi Pict "@!" Valid FunErrado( @cCodi, NIL, Row(), Col()+1 )
		@ 18, 11 Say "Limpar até o dia.:" Get dIni  Pict "##/##/##"
		Read
		if LastKey() = ESC
			ResTela( cScreen )
			return
	...
	Case Choice = 2
		MaBox( 16, 10, 18, 76 )
		@ 17, 11 Say "Limpar até o dia.:" Get dIni  Pict "##/##/##"
		Read
		if LastKey() = ESC
			ResTela( cScreen )
			return
