...
WHILE lOk
	Begin Sequence
      ...
      Case Op = 6.01 ; RecePosi( 1, NIL, cCaixa )
		Case Op = 6.02 ; RecePosi( 2, NIL, cCaixa )
		Case Op = 6.03 ; RecePosi( 3, NIL, cCaixa )
      Case Op = 6.04 ; RecePosi( 4, NIL, cCaixa )
      Case Op = 6.05 ; RecePosi( 5, NIL, cCaixa )
      Case Op = 6.06 ; RecePosi( 6, NIL, cCaixa )
      Case Op = 6.07 ; RecePosi( 1, NIL, cCaixa, OK )
      Case Op = 6.09 ; RecePosi( 1, NIL, cCaixa, NIL, (oAmbiente:cTipoRecibo := "RECBCO"))
      Case Op = 6.10 ; RecePosi( 1, NIL, cCaixa, NIL, (oAmbiente:cTipoRecibo := "RECOUT"))
      Case Op = 6.12 ; ReceGrafico()
      Case Op = 6.14 ; PosiAgeInd()
      Case Op = 6.15 ; PosiAgeReg()
      Case Op = 6.16 ; PosiAgeAll()
      Case Op = 6.17 ; SuporteIni()
      Case Op = 6.18 ; SuporteRecibo()
      ...
      Case op = 11.02 ; MudaCm()
      Case op = 11.03 ; MudaCm()
      Case op = 11.04 ; MudaCm()
      Case op = 11.05 ; MudaCm()
		EndCase
	End Sequence
EndDo
Mensagem("Aguarde, Fechando Arquivos." )
...
AADD( AtPrompt, {"R^eceber",     {"Consulta/Recibo Por Cliente",;
                                 "Consulta/Recibo Por Regiao",;
                                 "Consulta/Recibo Por Periodo",;
                                 "Consulta/Recibo Por Tipo",;
                                 "Consulta/Recibo Por Fatura",;
                                 "Consulta/Recibo Geral",;
                                 "Consulta/Recibo Para Rescisao",;
                                 "",;
                                 "Consulta/Recibo Recebido Banco",;
                                 "Consulta/Recibo Recebido Outros",;
                                 "",;
                                 "Grafico de Contas a Receber",;
                                 "",;
...
BEGIN
	...
	Case nChoice = 1
		cCodi := Space(05)
		MaBox( 11, 20, 13, 76 )
		@ 12, 21 Say "Cliente...:" Get cCodi Pict "99999" Valid RecErrado( @cCodi )
		Read
		IF LastKey() = ESC
			ResTela( cScreen )
			Loop
	...
	Case nChoice = 2
		cRegiao := Space(02)
		MaBox( 11, 20, 13, 36 )
		@ 12, 21 Say "Regiao..:" Get cRegiao Pict '99' Valid RegiaoErrada( @cRegiao )
		Read
		IF LastKey() = ESC
			ResTela( cScreen )
			Loop
...
While Eval( oBloco ) .AND. !Eof() .AND. Rel_Ok()
	IF Col >=  58
      ...
      Write( 02, 00, Padc( AllTrim(oAmbiente:xNomefir), Tam ) )
		Write( 03, 00, Padc( "SISTEMA DE CONTAS A RECEBER", Tam ) )
		Write( 04, 00, Padc( "EXTRATO DE CONTA", Tam ) )
		Write( 05, 00, Repl( SEP, Tam ) )
		Col := 6
	EndIF
	IF Col = 6
		Write(	Col, 00, NG + "CLIENTE....: " + Codi + " " + Receber->Nome + NR )
		Write( ++Col, 00, NG + "ENDERECO...: " + Receber->Ende + " " + Receber->Bair + NR )
		Write( ++Col, 00, NG + "CIDADE.....: " + Receber->Cep + "/"+ Receber->Cida + " " + "ESTADO..: " + Receber->Esta + Space( 07 ) + ;
									  "REGIAO..: " + Receber->Regiao + NR )
		Write( ++Col, 00, Repl( SEP, Tam ) )
		Write( ++Col, 00,"TITULO      TIPO     EMISSAO     VENCTO    LOCALIZACAO                     VALOR")
		Write( ++Col, 00, Repl( SEP, Tam ) )
		Col++
	EndIF
	IF NovoCodi
		...
		nAcumulado := 0
	EndIF
	Write( Col, 0, Docnr + "   " + Tipo + "   " + Dtoc( Emis ) + "   " + ;
						Dtoc( Vcto ) + "   " + Port + Space( 11 ) + Tran( Vlr, "@E 9,999,999,999.99" ) )
	Col++
	UltCodi		 := Codi
	nAcumulado	 += Vlr
	dUltCompra	 := Receber->UltCompra
	nMaiorAtraso := Receber->Matraso
	DbSkip()
	IF Col = 54 .OR. UltCodi != Codi
		Write( (Col + 2), 00, "*** Saldo Devedor *** ")
		Write( (Col + 2), ( MaxCol() - 15 ), Tran( nAcumulado, "@E 9,999,999,999.99" ))
		Write( (Col + 4), 00, NG + "MAC : " +  Tran( nAcumulado, "@E 9,999,999,999.99" ))
		Write( (Col + 4), 25, NG + "MATP : " + Tran( nMaiorAtraso, '999') + NR )
		Write( (Col + 4), 45, NG + "UC : " + Dtoc( dUltCompra ) + NR )
		nAcumulado := 0
		IF UltCodi != Codi
			NovoCodi := OK
		EndIF
...
WHILE OK
	...
	MaBox( 06 , 10 , 18 , 76, "ALTERACAO DE MOVIMENTO" )
	...
	cCodi := Recemov->Codi
	@ 07, 22 Get cCodi Pict PIC_RECEBER_CODI Valid RecErrado( @cCodi )
	Read
	IF LastKey() = ESC
		DbClearFilter()
		ResTela( cScreen )
...
LOCAL cDolar	  := "R"
LOCAL aRecno	  := {}
LOCAL aMenu      := {"Duplicata Form Branco","Duplicata Form Personalizado","Boleto Bancario","Promissoria Papel Branco","Promissoria Papel Continuo"}
...
WHILE OK
	...
	oMenu:Limpa()
	MaBox( 05 , 10 , 07 , 26 )
	@ 06 , 11 Say	"Cliente.:" Get cCodi Pict PIC_RECEBER_CODI Valid RecErrado( @cCodi ) .AND. VerificaPosicao( cCodi )
	Read
	IF LastKey() = ESC
		ResTela( cScreen )
		Exit
...
WHILE OK
	...
	xNtx	 := FTempName("T*.$*")
	M_Title( "FICHA/RELACAO CLIENTES" )
	nChoice := FazMenu( 08 , 20 ,  AtPrompt, Cor())
	Do Case
	Case nChoice = 0
		ResTela( cScreen )
		Exit
...
WHILE OK
	...
	M_Title( "FICHA/RELACAO CLIENTES" )
	nChoice := FazMenu( 10 , 20 ,  AtPrompt, Cor())
	Do Case
	Case nChoice = 0
		ResTela( cScreen )
		Exit
...
LOCAL dIni		:= Date()-30
LOCAL dFim		:= Date()
LOCAL AtPrompt := { "Individual ", "Por Regiao", "Por Cidade", "Por Estado", "Geral", "Data Abertura", "Quantitativo de Clientes", "Com Debito e Negativado SPC", "Sem Debito e Negativado SPC", "Com Debito e Positivado SPC", "Clientes com Contrato Suporte", "Com Contrato Suporte Vencido", "Ultima Compra" }
...
WHILE OK
	...
	Case nChoice = 10 // Com Debito e Positivado SPC
		...
		MaBox( 19 , 20 , 22 , 78 )
		@ 20 , 21 Say	"Cliente.:" Get cCodi Pict PIC_RECEBER_CODI Valid RecErrado( @cCodi,, Row(), Col()+1 )
		@ 21 , 21 Say	"Cliente.:" Get cCodiFim Pict PIC_RECEBER_CODI Valid RecErrado( @cCodiFim,, Row(), Col()+1 )
		Read
		IF LastKey( ) = ESC
			ResTela( cScreen )
			Loop
...
ErrorBeep()
lFanta := Conf('Pergunta: Imprimir nome Fantasia ?')
Regiao->(Order( REGIAO_REGIAO ))
IF Regiao->(DbSeek( cRegiao))
	cNome := Regiao->Nome
EndIF
...
WHILE !Eof() .AND. Rel_Ok()
	IF Col >= 57
      ...
      Write( 02 ,  000, Padc( AllTrim(oAmbiente:xNomefir), Tam))
		...
		Write( 05 ,  000, Repl( SEP, Tam ))
		Write( 06 ,  000, "CODI  NOME                                     ENDERECO                       BAIRRO   CIDADE                    UF       TELEFONE S")
		Write( 07 ,  000, Repl( SEP, Tam ))
		Col := 8
	EndIF
	IF nOpcao = 6 // Fantasia
		Qout( Codi, Fanta, Ende, Left( Bair, 8), Cida, Esta, Fone, IF( Cancelada, "C", "A"))
		IF lFanta == OK
			Qout( Space(05), Nome, Obs)
			Col++
		EndIF
	Else
		Qout( Codi, Nome, Ende, Left(Bair,8), Cida, Esta, Fone, IF( Cancelada, "C", "A"))
		IF lFanta == OK
			Qout( Space(05), Fanta, Obs)
			Col++
		EndIF
	EndIF
	Col		  ++
...
Proc ReciboDiv( cCaixa, cVendedor )
***********************************
LOCAL cScreen := SaveScreen()
...
Proc DivDiaria( cCaixa, cVendedor, cTipo )
******************************************
LOCAL Arq_Ant	 := Alias()
LOCAL Ind_Ant	 := IndexOrd()
LOCAL cScreen	 := SaveScreen()
...
WHILE OK
   ...
   IF Recibo->(Incluiu())
		Recibo->Nome	 := cNome
      Recibo->Vcto    := Date()
      Recibo->Tipo    := IF( nChoice = 2, "RECDIV", "VALDIV")
      Recibo->Codi    := "00000"
		Recibo->Docnr	 := cDocnr
      Recibo->Hora    := Time()
      Recibo->Data    := Date()
...
oBrowse:HotKey( F4, {|| NIL } )
oBrowse:PreDoGet := {|| PreDoAge( oBrowse ) }
oBrowse:PosDoGet := NIL
oBrowse:PreDoDel := NIL
oBrowse:PosDoDel := NIL
oBrowse:Show()
...
WHILE OK
	...
	@ 14, 02 Say "Hist.:"             GET cObs     PICT '@!'             Valid IF(Empty(cObs), ( ErrorBeep(), Alerta("Ooops!: Entre com o historico!"), FALSO ), OK )
	@ 15, 02 Say "Prox Agendamento.:" GET dDataPro PICT "##/##/##"
	@ 16, 02 Say "Hist.:"             GET cObs1    PICT '@!'             Valid IF(!Empty(dDataPro) .AND. Empty(cObs1), ( ErrorBeep(), Alerta("Ooops!: Entre com o historico!"), FALSO ), OK )
	Read
	IF LastKey() = ESC
		ResTela( cScreen )
		Return
...
Proc BidoGrafico(nOp)
*********************
LOCAL cScreen := SaveScreen()
LOCAL aMenu   := {"Por Cliente","Geral"}
LOCAL cScreen1
LOCAL nChoice
