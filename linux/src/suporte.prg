/*
  ┌─────────────────────────────────────────────────────────────────────────┐
 ▌│																								 │
 ▌│	Programa.....: SUPORTE.PRG 														 │
 ▌│	Aplicacaoo...: MODULO DE SUPORTE AO SCI										 │
 ▌│	Versao.......: 19.50 																 │
...
WHILE OK
	...
	cString1 := " DOCTO Nº     EMIS   VENCTO ATRA  NOMINAL     DESC    MULTA    JUROS    GERAL OBSERVACOES" 
...
LOCAL cObs      := "APOS " + AllTrim(Str(nDiaMulta,3)) + "º DIA, " + AllTrim(Str(nMulta, 6, 2)) + "% MULTA + JUROS AO DIA"
LOCAL nLargura  := 53
...
oBrowse:Titulo   := "CONSULTA/ALTERACAO DE COTAÇAO DE DOLAR"
oBrowse:PreDoGet := NIL
oBrowse:PosDoGet := NIL
oBrowse:Show()
oBrowse:Processa()
...
FWriteLine( Handle,"NrDocumento=05,12                   ; Linha,Coluna da Nº do Documento")
...
LOCAL cObs1 			:= "PROTESTAR NO 5º DIA UTIL APOS VENCIDO.  "
...
WHILE OK
	...
	Case nChoice = 11
		 ErrorBeep()
		 IF Conf("Pergunta: Continuar com a operaçao ?")
			 DupSaidas()
			 Fechatudo()
...
IF Conf('Pergunta: Deseja imprimir ?')
	...
	For nX := 12 To 1 Step -1
		...
		nTotal := m[nX,1]
		Receber->(DbSeek( cCodi ))
		Qout( cCodi, Receber->Nome, Tran( nTotal, '@E 999,999,999.99'), Tran( nConta, '99º' ))
		oRelato:RowPrn ++
	Next
	__Eject()
	PrintOff()
...
Write(nRow+08, 0, '│   Valor      Numero   │   Valor      Nº de Ordem   │             │Inst. Fin. │')
...
Write(nRow+28, 0, '│Data do Aceite : ___/___/_____               ──────────────────────────────── │')
...
WHILE OK
	...
	@ Row()+1, nCol Say "Documento Nº......:" Get cDocnr Pict "@K!"      Valid PgDocCerto( @cDocnr )
	...
	IF Conf('Pergunta: Confirma Inclusao do Registro ?')
		IF Pagamov->(Incluiu())
			...
			*:-------------------------------------------------------
			...
			Write(nRow+03, 0, '│' + xcEnde +                                   '│         Nº :' + cDocnr +   '│')
			Write(nRow+04, 0, '│' + xcFone +                                   '│                             │')
			Write(nRow+05, 0, '│' + xcCepCida +                                '│DATA DE EMISSAO : '+ cEmis +'│')
			Write(nRow+06, 0, '├────────────────────────────────────────────────┴─────────────────────────────┤')
			...
			Write(++nRow, 0, '│     sendo o uso para outras mercadorias ou fornecedor         └──────────────┤')
			Write(++nRow, 0, '│                                                                              │')
			Write(++nRow, 0, '│                                                                              │')
