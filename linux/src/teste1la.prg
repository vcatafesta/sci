/*
  ┌─────────────────────────────────────────────────────────────────────────┐
 ▌│																								 │
 ▌│	Programa.....: TESTE1LA.PRG														 │
 ▌│	Aplicacaoo...: SISTEMA DE CONTROLE DE ESTOQUE								 │
 ▌│	Versao.......: 19.50 																 │
...
For I := 1 To nCop
	...
	PrintOff()
	Mensagem( "Aguarde ... Imprimindo Copia Nº " + StrZero(I,4), WARNING )
	PrintOn()
	Tam	  := 132
	FPrint( PQ )
	SetPrc( 0, 0 )
...
FPrint( PQ )
SetPrc( 0, 0 )
Write( 01, 00, Padr( "Pagina Nº 001", ( Tam/2 ) ) + Padl( Time(), ( Tam/2 ) ) )
Write( 02, 00, Date() )
...
WHILE Eval( oBloco ) .AND. Rel_Ok()
	if Col >= 57
		 Write( 00, 00, "Pagina Nº " + StrZero( ++Pagina, 3 ) )
...
While !Eof() .AND. REL_OK()
	if Col >=  58
		Write( 01, 00, Padr( "Pagina Nº " + StrZero( ++Pagina,3 ), ( Tam/2 ) ) + Padl( Time(), ( Tam/2 ) ) )
		Write( 02, 00, Date() )
		Write( 03, 00, Padc( XNOMEFIR, Tam ) )
...
WHILE OK
	...
	elseif nChoice = 4
		...
		WHILE OK
			...
			cDocnr := Space(09)
			MaBox( 14, 12, 16, 44 )
			@ 15, 13 Say 'Nº Nota Entrada..:' Get cDocnr Pict "@!" Valid AchaDocEntrada( @cDocnr )
			Read
			if LastKey() = ESC
				ResTela( cScreen )
				Exit
...
WHILE OK
	 ...
	 MaBox( 15, 10, 17, 37 )
	 cFatura := Space(7)
	 @ 16, 11 Say "Fatura Nº.....»" Get cFatura Pict "@!" Valid VisualAchaFatura( @cFatura )
	 Read
	 if LastKey() = K_ESC
		 ResTela( cScreen )
		 Exit
...
**********************************************
Write( 00, 00, Padr( "Pagina Nº " + StrZero( Pagina, 4 ), ( Tam/2 ) ) + Padl( Time(), ( Tam/2 ) ) )
Write( 01, 00, Date() )
Write( 02, 00, Padc( XNOMEFIR, Tam ) )
Write( 03, 00, Padc( SISTEM_NA2, Tam ) )
Write( 04, 00, Padc( Titulo ,Tam ) )
...
oSaidas:AddColumn( TBColumnNew( "FATURA Nº", {||Fatura } ) )
oSaidas:AddColumn( TBColumnNew( "PEDIDO Nº", {||Pedido } ) )
...
DbGotop()
oRecemov:AddColumn(TBColumnNew( "TITULO Nº ", {|| DocNr } ))
...
Write( 05, 00, Repl( SEP, Tam ))
Write( 06, 00, "DATA     DOCTO Nº  TIPO   NOME DO CLIENTE      CODIGO DESCRICAO DO PRODUTO                      QUANT      P. VAREJO  PRECO VENDIDO     T. VENDIDO FP")
Write( 07, 00, Repl( SEP, Tam ))
return
...
Case nChoice = 3
	...
	WHILE OK
		cFatura := Space( 07 )
		MaBox( 20, 01, 22, 25 )
		@ 21, 02 Say "Fatura Nº...:" Get cFatura Pict "@!" Valid VisualAchaFatura( @cFatura )
		Read
		if LastKey() = ESC
			ResTela( cScreen )
			Exit
...
nRow := 45
Write( nRow++, 00, "DCTO Nº   TP     EMISSAO  VENCTO   PORTADOR    VLR NOMINAL DT PAGTO     VLR PAGO STATUS")
Write( nRow++, 00, Repl( SEP, Tam ) )
...
return
   ...
   if cNoFatura = NIL
	...
	if Len( aFatura ) > 0
		...
		@ nLinha+01, 02 Say "Nº Nota Fiscal......:" Get cFatura        Pict "@!" When lSequencia = "S"
   ...
   if( anT[XTRANSPORTADOR+07,01] >= 0, Write( anT[XTRANSPORTADOR+07, 01], anT[XTRANSPORTADOR+07, 02],  aTrans[27] ),)
	...
	if nRow != 0 .AND. nCol != 0
		 ...
		 if Recemov->(DbSeek( cFaturaSub ))
			 DevPos( nRow, nCol ) ; DevOut("DOCTO Nº    VENCTO            VALOR")
...
Write( 03, 00 , "Fatura Nº..: " + cFatura )
Write( 03, 66 , "Fatura Nº..: " + cDescarga )
Forma->(Order( FORMA_FORMA))
Forma->(DbSeek( cForma ))
