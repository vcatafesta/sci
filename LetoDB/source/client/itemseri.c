...
static void * hb_arrayId( PHB_ITEM pArray )
{
      ...
      case HB_IT_MEMO:
         ...
         else
         {
            ...
            hb_cdpnDup2( szVal, nSize, ( char * ) &pBuffer[ nOffset ], &n,
                         cdpIn, cdpOut );
#else
            memcpy( ( char * ) &pBuffer[ nOffset ], szVal, nSize );
#endif
            nOffset += nLen;
         }
         break;
...
#else
         szVal = hb_xgrab( nLen + 1 );
         memcpy( szVal, ( const char * ) &pBuffer[ nOffset ], nLen);
         hb_itemPutCL( pItem, szVal, nLen );
         hb_xfree( szVal );
#endif
         nOffset += nSize;
         break;
...
#else
         memcpy( szVal, ( const char * ) &pBuffer[ nOffset ], nSize);
#endif
         memset( szVal + nLen, ' ', ulPad );
#ifndef CDP_OFF
         hb_itemPutCLPtr( pItem, szVal, nLen + ulPad );
#else
         hb_itemPutCL( pItem, szVal, nLen + ulPad );
         hb_xfree( szVal );
#endif
         nOffset += nSize;
         break;
...
#else
      HB_SYMBOL_UNUSED( pszCdpIn );
      HB_SYMBOL_UNUSED( pszCdpOut );
      cdpIn = cdpOut = NULL;
#endif
