...
Write( 05, 1 , "Fatura Nº...:          │Emissao...:")
Write( 06, 1 , "Desconto %..:          │Liquido...:")
Write( 07, 1 , "═══════════════════════╪═══════════════════─DADOS DO CLIENTE─═════════════════")
Write( 08, 1 , "Cliente.....:          │                                                      ")
Write( 09, 1 , "C/C/........:          │                                                      ")
Write( 10, 1 , "Placa.......:          │                                                      ")
...
if Instru80() .OR. Lptok()
	...
	Write( nRow + 10, 15, "CONFORME ORCAMENTO Nº " + cFatu )
...
Qout( GD + Padc( "ORCAMENTO Nº " + cFatu, Tam/2 ) + CA )
...
if Tam = 66
	...
	if lMarcaNoTicket = OK
		...
		if nTipoBusca = 1 // Codigo
			...
			Qout( "Nº ORIGINAL        QUANT DESCRICAO DO PRODUTO                             UNITARIO      TOTAL")
		endif
	endif
endif
Qout( Repl("-", Tam))
	...
	if nCol + nRodape >= nTamForm
		if (xAlias)->(!Eof())
			if nCol >= ( nTamForm - 3 )
				...
				Qout( Repl("-", Tam))
				Qout( GD + Padc( "ORCAMENTO Nº " + cFatu, Tam/2 ) + CA)
				Qout( "Nº Docto.: " + cFatu, Space(nDif+31), "Data : " + Dtoc( dEmis ))
				Qout( Repl("-", Tam))
				if Tam = 66
					Qout( "CODIGO    QUANT DESCRICAO DO PRODUTO                         TOTAL")
				else
...
if Recemov->(DbSeek( cFatu ))
	Qout( "DOCTO Nº  TIPO     VCTO       JR DIA     VLR TITULO")
...
While (xAlias)->(!Eof())
	if nCol >= nLinhas
      Write( 00, 00, GD + Padc( AllTrim(oAmbiente:xNomefir) - ' - ' + XFONE, Tam/2 ) + CA)
		...
		Fprint( PQ )
		Write( 03, 000, "Nº Docto.: " + cFatu  + " Caixa.: " + cCaixa + " Data.: " + Dtoc( dEmis ) + " Hora..: " + Time())
...
While (xAlias)->(!Eof())
	if nCol >= nLinhas
		...
		Fprint( PQ )
		Write( 01, 000, "Nº Docto.: " + cFatu  + " Caixa.: " + cCaixa + " Data.: " + Dtoc( dEmis ) + " Hora..: " + Time())
...
Qout(  "Nº Docto.: ", cFatu, Space(29), "Data : ", Dtoc( dEmis ))
Qout(  "Cliente..: ", cCodi, cNomeCliente )
Qout(  "Endereco.: ", Receber->Ende, Receber->Fone )
Qout(  Repl("-", Tam))
Qout(  "DOCTO Nº              VENCTO                     VALOR")
Qout(  Repl("-", Tam))
nCol := 08
...
WHILE OK
	...
	@ 16, 11 Say "Docto. Nº.:»" Get cDocnr   Pict "@K!" Valid CheqDoc( cDocnr ) .OR. LastKey() = UP
...
def xIncluiRegistro( lVarejo, oObjeto, lFatCodeBar )
   ...
   WHILE OK
      ...
      if !lFatCodeBar
         ...
         if lSerieProduto	// Entrar com nº serie Produto ?
...
oMenu:Limpa()
MaBox( 18, 10, 20, 32 )
@ 19, 11 Say "Fatura Nº...»" Get cFatura Pict "@!" Valid VisualAchaFatura( @cFatura )
Read
if LastKey() = ESC
	ResTela( cScreen )
	return
...
SetPrc(0,0)
Write(	nCol, 00, "")
Write( ++nCol, 00, NG + Padc("CONTRATO PARTICULAR DE COMPRA E VENDA COM RESERVA DE DOMINIO - Nº " + cFatu, Tam ) + NR )
Write( ++nCol, 00, Repl("-", Tam ))
...
Write( ++nCol, 00, Repl("-", Tam ))
Write( ++nCol, 00, "Nº DOCTO  VENCIMENTO       VALOR OBS                 Nº DOCTO  VENCIMENTO       VALOR OBS")
nLen := Len( Dpnr )
nSoma := 0
nSum	:= 1
For nY := 1 To nLen
...
if lMostrar = Nil
	MaBox( nPos+2, 00, MaxRow()-1, MaxCol(),"TOTAL FATURA R$  " + Tran( nTotal, "@E 9,999,999,999.99"))
	Write( nPos+3, 01,"ENTER   Incluir Registros▌  ^ENTER Alterar          ▌  ^Q  Limpar Fatura    ")
	Write( nPos+4, 01,"DELETE  Excluir Registros▌  F10    Fechar Fatura    ▌  F11 Devolucao Total  ")
	Write( nPos+5, 01,"F5      Lista Precos     ▌                          ▌                       ")
	Write( nPos+6, 01,"                         ▌                          ▌                       ")
	Write( nPos+7, 01,"                         ▌                          ▌                       ")
endif
return
...
if lMostrar = NIL
	MaBox( 17, 00, MaxRow(), MaxCol(),"TOTAL COMPRA R$ " + Tran( nTotal, "@E 9,999,999,999.99") + " ▌ TOTAL CUSTO R$  " + Tran( nTotCustoFinal, "@E 9,999,999,999.99"))
	Write( 18, 01, "INSERT Incluir Registro  ▌  F1  Posicao Cliente    ▌  F2  Recebimentos      ")
	Write( 19, 01, "DELETE Excluir Registro  ▌  F3  Pagamentos         ▌  F4  Alterar Produtos  ")
	Write( 20, 01, "^ENTER Alterar Registro  ▌  F5  Lista Precos       ▌  F6  Consulta Entradas ")
	Write( 21, 01, "^Q     Limpar Fatura     ▌  F7  Alterar Fornecedor ▌  F8  Posicao Fornecedor")
	Write( 22, 01, "F10    Fechar Fatura     ▌  F9  Incluir Fornecedor ▌  F11 Devolucao Fatura  ")
	Write( 23, 01, "F12                      ▌ ^F1  Alteracao Pagar    ▌ ^F2  Alteracao Pago    ")
endif
return
...
if lMostrar = Nil
	MaBox( 17, 00, MaxRow(), MaxCol(),"TOTAL COMPRA R$ " + Tran( nTotal, "@E 9,999,999,999.99") + " ▌ TOTAL CUSTO R$  " + Tran( nTotCustoFinal, "@E 9,999,999,999.99"))
	Write( 18, 01,"--+    Incluir Registros▌  ^--+ Alterar           ▌  ^Q  Limpar Fatura    ")
	Write( 19, 01,"DELETE  Excluir Registros▌  F10   Fechar Fatura     ▌  F11 Devolucao Total  ")
	Write( 20, 01,"F5      Lista Precos     ▌                          ▌                       ")
	Write( 21, 01,"                         ▌                          ▌                       ")
	Write( 22, 01,"                         ▌                          ▌                       ")
	Write( 23, 01,"                         ▌                          ▌                       ")
endif
...
Write( 13, 1, Repl( "─", MaxCol()-1))
Write( 14, 1, "  TITULO Nº        VALOR DIAS VENCTO    TIPO  JR/MES  DESC PORTADOR  " )
Write( 15, 1, Repl( "─", MaxCol()-1))
Write( 16, 1, "A:                                                                           " )
Write( 17, 1, "B:                                                                           " )
Write( 18, 1, "C:                                                                           " )
Write( 19, 1, "D:                                                                           " )
Write( 20, 1, "E:                                                                           " )
Write( 21, 1, Repl( "─", MaxCol()-1))
Write( 22, 1, "OBS:                                                                         " )
Write( 23, 1, "                                                                             " )
...
WHILE OK
	...
	aDevolucao := {}
	MaBox( 18, 10, 20, 32 )
	@ 19, 11 Say "Fatura Nº...:" Get cFatura Pict "@!" Valid VisualEntraFatura( @cFatura )
	Read
	if LastKey() = ESC
		ResTela( cScreen )
		return
...
*********************************************************
Write( 00, 00, Padr( "Pagina Nº " + StrZero( Pagina,3 ), ( Tam/2 ) ) + Padl( Time(), ( Tam/2 ) ) )
Write( 01, 00, Date())
			...
			if Saidas->(Dbseek( cCodi ))
				...
				if Cheque->(DbSeek( cCodiCx )) .OR. !Empty( cCodiCx )
					if Chemov->(Incluiu())
						...
						Chemov->Data	  := dData
						Chemov->Baixa	  := Date()
						Chemov->Hist	  := "CRE REF FATURA Nº " + cFatura
						Chemov->Saldo	  := nChSaldo
						Chemov->Tipo	  := "DH"
...
Case nChoice = 2
	WHILE OK
		...
		@ 12, 11 Say "Fatura nº.......: " Get cFatura Pict "@!"   When BaixaLocaliza( cCodi, @cFatura ) Valid VisualAchaFatura( @cFatura, @nApagar, @cCodi )
...
WHILE OK
	...
	@ 06, 	  01 Say "Documento nº....:" Get cFaturaPrevenda Pict "@!"    Valid VerNrPrevenda( @cFaturaPrevenda, @cForma, @cCodiVen, @cCodi, @cNome, @cEnde, @cFone, @cAparelho, @cMarca, @cModelo, @cNrSerie, @cObs, @cObs1, @cObs2, @cAno, @cCor, @cPlaca, @cEstadoGeral )
	...
	if nCol + nRodape >= nTamForm
		if (xAlias)->(!Eof())
			if nCol >= ( nTamForm - 3 )
				...
				Qout( Repl("-", Tam))
				Qout( GD + Padc( "ORCAMENTO Nº " + cFatu, Tam/2 ) + CA)
				Qout( "Nº Docto.: " + cFatu, Space(nDif+28), "Data : " + Dtoc( dEmis ))
				Qout( Repl("-", Tam))
				if Tam = 66
					Qout( "CODIGO    QUANT DESCRICAO DO PRODUTO                         TOTAL")
				else
...
def ZerarPreVenda()
	...
	WHILE OK
		...
		@ SCI_MAXROW - 5 , 11 Say "Nº Pre-Venda a Excluir.:" Get cFaturaPrevenda Pict "@!" Valid AchaPreVenda( @cFaturaPrevenda, OK )
		Read
		if LastKey() = ESC
			AreaAnt( Arq_Ant, Ind_Ant )
			ResTela( cScreen )
...
WHILE OK
	...
	AchaPreVenda( @cFaturaPrevenda, FALSO )
	@ nPos+4, 11 Say "Pre-Venda nº.:" Get cFaturaPrevenda Pict "@!" Valid AchaPreVenda( @cFaturaPrevenda )
	Read
	if LastKey() = ESC
		AreaAnt( Arq_Ant, Ind_Ant )
		ResTela( cScreen )
...
WHILE OK
	...
	cFatu 	  := Space(07)
	MaBox( 18, 10, 20, 34 )
	@ 19, 11 Say "Fatura nº....:" Get cFatu Pict "@!" Valid VisualAchaFatura( @cFatu )
	Read
	if LastKey() = ESC
		AreaAnt( Arq_Ant, Ind_Ant )
		ResTela( cScreen )
...
WHILE OK
	...
	cFatu 	  := Space(07)
	MaBox( 18, 10, 20, 34 )
	@ 19, 11 Say "Ticket nº....:" Get cFatu Pict "@!" Valid VisualAchaFatura( @cFatu )
	Read
	if LastKey() = ESC
		AreaAnt( Arq_Ant, Ind_Ant )
		ResTela( cScreen )
...
WHILE OK
	...
	cFaturaFim := Space(07)
	MaBox( 17, 10, 20, 41 )
	@ 18, 11 Say "Pre-Venda nº Inicial.:" Get cFaturaIni Pict "@!" Valid AchaPreVenda( @cFaturaIni )
	@ 19, 11 Say "Pre-Venda nº Final...:" Get cFaturaFim Pict "@!" Valid AchaPreVenda( @cFaturaFim )
	Read
	if LastKey() = ESC
		AreaAnt( Arq_Ant, Ind_Ant )
		ResTela( cScreen )
...
Case nChoice = 3
	...
	WHILE OK
		cFatura := Space( 07 )
		MaBox( 20, 01, 22, 25 )
		@ 21, 02 Say "Fatura Nº...:" Get cFatura Pict "@!" Valid AchaPrevenda( @cFatura )
		Read
		if LastKey() = ESC
			ResTela( cScreen )
			Exit
...
Case nChoice = 5
	ErrorBeep()
	if !Conf("Pergunta: Poderá demorar. Continuar ?")
		ResTela( cScreen )
		return
	endif
	nRecno := 0
...
Write( ++nCol, 00, "sao de 1º qualidade, e apropriada para o fim que se destina.")
nCol++
Write( ++nCol, 00, NG + "SEXTA: " + NR + "Na vigencia deste contrato nao podera o 'COMPRADOR' alienar sob  qualquer  forma, dar")
   ...
   restela( cTela1 )
	if xDbfCaixa->(Lastrec()) > 0		      
      ...
      Mensagem("Aguarde, imprimindo relatorio detalhe de caixa")
		...
		if lDetalhe
			Write( 06, 00,"TIPO   ORC/FAT   VENCTO   PAGTO    HISTORICO/CLIENTE                DOCTO Nº       NOMINAL       DEBITO      CREDITO         DF   CX")
		...
		if lDetalhe
			...
			WHILE xDbfCaixa->(!Eof()) .AND. Rel_Ok()
				if nCol >= 58
					__Eject()
					CabecCaixa( ++Pagina, Tam, cTitulo, cCaixa, cTitular )
					Write( 08, 00,"TIPO   ORC/FAT   VENCTO   PAGTO    HISTORICO/CLIENTE                DOCTO Nº       NOMINAL       DEBITO      CREDITO         DF   CX")
      ...
      AlertaPy("Informa: Nenhum registro atende a condiçao.")
   endif      
	Recemov->(DbClearRel())
	Recemov->(DbGoTop())
	xDbfCaixa->(DbCloseArea())
...
SetPrc(0,0)
Write(	nCol, 00, "")
Write( ++nCol, 00, NG + Padc("CONTRATO PARTICULAR DE CONFISSAO DE DIVIDA E COMPROMISSO DE PAGAMENTO - Nº " + cFatu, Tam ) + NR )
Write( ++nCol, 00, Repl("-", Tam ))
Write( ++nCol, 00, NG + "CREDORA: " + NR + AllTrim(AllTrim(oAmbiente:xNomefir)) + ", empresa de direito privado, estabelecida na")
Write( ++nCol, 00, XENDEFIR + "/" + XCCIDA + "-" + XCESTA + ",inscrita no CPF/CNPJ nº " + XCGCFIR )
...
Write( ++nCol, 00, "inscrita no CPF/CNPJ sob nº " + cCpf + " RG/IE : " + cRg )
Write( ++nCol, 00, Repl("-", Tam ))
...
Write( ++nCol, 00, Repl("-", Tam ))
Write( ++nCol, 00, "Nº DOCTO  VENCIMENTO       VALOR                     Nº DOCTO  VENCIMENTO       VALOR")
nLen := Len( Dpnr )
nSoma := 0
nSum	:= 1
For nY := 1 To nLen
...
SetPrc(0,0)
Write(	nCol, 00, "")
Write( ++nCol, 00, NG + Padc("INSTRUMENTO PARTICULAR DE CONTRATO DE COMPRA E VENDA COM RESERVA DE DOMINIO - Nº " + cFatu, Tam ) + NR )
Write( ++nCol, 00, Repl("-", Tam ))
nCol++
...
Write( ++nCol, 00, "te inscrita no CGC/MF sob o nº 15.875.594/00001-51, Inscricao Estadual nº 407.15974-3, simples-")
...
nCol++
Write( ++nCol, 00, "1ª) - O preco de venda e de R$ " + AllTrim( Tran( nLiquido, '@E 999,999,999.99')) + ' cujo pagamento o COMPRADOR se obriga a realizar')
Write( ++nCol, 00, 'do seguinte modo:')
Write( ++nCol, 00, Repl("-", Tam ))
Write( ++nCol, 00, "Nº DUPLICATA         VENCIMENTO                  VALOR       OBS")
Write( ++nCol, 00, Repl("-", Tam ))
nLen := Len( Dpnr )
nSoma := 0
For nY := 1 To nLen
...
Write( ++nCol, 00, "as quais ficam fazendo parte integral no presente instrumento.")
nCol++
Write( ++nCol, 00, "2ª) - Por forca do pagamento de reserva de dominio, aqui expressamente instituido, e aceito pe-")
...
nCol++
Write( ++nCol, 00, "3ª) - Em consequencia do disposto na Clausula precedente, caso faltar o COMPRADOR, ao   pontual")
...
nCol++
Write( ++nCol, 00, "4ª) - A falencia do COMPRADOR tambem resolve este contrato, podendo a VENDEDORA reivindicar  da")
Write( ++nCol, 00, "massa o(s) objeto(s) condicionalmente vendido(s).")
nCol++
Write( ++nCol, 00, "5ª) - Enquanto nao tiver pago integralmente o preco, fica expressamente proibido o COMPRADOR, a")
