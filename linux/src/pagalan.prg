/*
 ...
 ▌│																								 │
 ▌│	Modulo.......: PAGALAN.PRG		   												 │
 ▌│	Sistema......: CONTROLE DE CONTAS A PAGAR						             │ 
...
if Pagar->(!DbSeek( cCodi ))
	...
	Pagar->(Escolhe( 03, 01, 22,"Codi + '▌' + Nome + '▌' + Sigla + '▌' + Fone", "CODI NOME FORNECEDOR                          SIGLA      TELEFONE", aRotina,, aRotinaAlteracao ))
endif
...
Vetor3 := { "Emis","Vcto","Docnr","Tran( Vlr, '@E 999,999,999,999.99')","DataPag",;
				...
				"Tipo","Port"}
Vetor4 := { "EMISSAO","VCTO", "DOCTO Nº", "VALOR","DATA PGTO","VALOR PAGO","JUROS","TIPO","PORTADOR" }
...
WHILE OK
  ...
  @ 09, 06 Say "Doc Nº...:" Get cDocnr    Pict "@!"   Valid PgDocCerto( cDocNr, cSwap )
...
if Pagar->(!DbSeek( cCodi ))
	Pagar->(Order( PAGAR_NOME ))
	Pagar->(Escolhe( 03, 01, 18, "Codi + '║' + Nome + '║' + Fone","CODI NOME FORNECEDOR                          TELEFONE", aRotina ))
endif
cCodi := Pagar->Codi
if nRow != NIL
	Write( nRow, nCol, Pagar->Nome )
...
WHILE OK
	...
	WHILE OK
		...
		nCol		  := 03
		MaBox( 07, 02, 21, 77 )
		@ nRow,	  nCol Say "Fatura Nº..:" Get cFatura    Pict "@K!"
		@ Row()+1, nCol Say "Vlr Fatura.:" Get nVlrFatura Pict "@E 9,999,999,999.99"
		@ Row()+1, nCol Say "Tipo.......:" Get cTipo      Pict "@K!"
		@ Row()+1, nCol Say "Docto Nº...:" Get cDocnr     Pict "@K!" Valid PgDocCerto( @cDocnr )
...
WHILE !Eof() .AND. Rel_Ok()
	...
	if Col >= 57
		Write( 00, 001, Padr( "Pagina Nº " + StrZero( ++Pagina,3 ), ( Tam/2 ) ) + Padl( Dtoc( Date()) + " - " + Time(), ( Tam/2 ) ) )
		Write( 01, 000, Padc( XNOMEFIR, Tam ))
		Write( 02, 000, Padc( SISTEM_NA4, Tam ))
		Write( 03, 000, Padc( cTitulo , Tam ))
		Write( 04, 000, Repl( "=", Tam ) )
		Write( 05, 000, "DOCTO Nº  TP      EMISSAO   VENCTO PORTADOR        VALOR JR MES  DESC   ATR     JR DIA      JUROS  VLR+JUROS             OBSERVACOES")
...
WHILE OK
	  ...
	  if Len( aTodos ) > 0
		  ...
		  MaBox( 07, 00, 23, 79, "DOCTO Nº   EMISSAO  VENCTO  ATRA    NOMINAL   DESCONTO      JUROS      TOTAL ")
		  M_Title("[ESC] RETORNA")
		  __FuncaoPago( 0, 1, 1 )
...
WHILE OK
	  ...
	  if Len( aTodos ) > 0
		  ...
		  MaBox( 07, 00, 23, 79, " DOCTO Nº  EMISSAO   VENCTO  DATAPAG ATRA   VALOR TITULO      VALOR PAGO            " )
		  M_Title("[ESC] RETORNA")
		  __FunPago( 0, 1, 1 )
...
WHILE OK
	cFatura := Space(07)
	MaBox( 14, 55, 16, 79 )
	@ 15, 56 Say "Fatura Nº..:" Get cFatura Pict "@!" Valid VisualEntraFatura( @cFatura )
	Read
	if LastKey() = ESC
		ResTela( cScreen )
		return
	...
	if Len( aTodos ) > 0
		...
		MaBox( 07, 00, 23, 79, " DOCTO Nº  EMISSAO   VENCTO  DATAPAG ATRA   VALOR DEVIDO     VALOR PAGO             " )
		MaBox( 07, 00, 23, 79, " DOCTO Nº  EMISSAO   VENCTO  DATAPAG ATRA    NOMINAL    A PAGAR       PAGO          " )
		M_Title("[ESC] RETORNA")
		__FunPP( 0, 1, 1 )
...
if Pago->(!DbSeek( cDocnr ))
	...
	Pago->(Escolhe( 03, 01, 22, "Docnr + '║' + Pagar->Nome", "DOCTO Nº  NOME DO FORNECEDOR"))
	Pago->(DbClearRel())
endif
cDocnr := Pago->Docnr
AreaAnt( Arq_Ant, Ind_Ant )
...
WHILE !Eof() .AND. Rel_Ok()
	...
	if Col >= 57
		Pagina++
		Write( 00, 000, "Pagina Nº " + StrZero( Pagina, 3 ) )
		...
		Write( 06, 000, "CODI NOME DO FORNECEDOR        TIPO    DOCTO Nº   PORTADOR EMISSAO    VENCTO     VALOR TITULO ATRASO       JURO/DIA      VALOR+JUROS")
		Write( 07, 000, Repl( SEP, Tam ) )
		Col := 8
	endif
...
cRelato	+= cString
cCabeca	:= "CODI NOME DO FORNECEDOR           TIPO     DOCTO  Nº EMISSAO    VENCTO     VALOR TITULO DATA PGT   ATR       VALOR PAGO PORTADOR  ND"
Mensagem("Aguarde... Imprimindo. ESC Cancela.", Cor())
PrintOn()
FPrint( PQ )
SetPrc( 0,0 )
