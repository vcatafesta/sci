...
METHOD TReceposi:New(nBoxRow, nBoxCol, nBoxRow1, nBoxCol1, cTop, cBottom)
...
METHOD TReceposi:PrintPosi( nRow, nCol )
	::nPrtRow := nRow
	::nPrtCol := nCol
...
METHOD TReceposi:RegistroEmBranco(cCodi,cFatu)
...
METHOD TReceposi:DeleteReg(nReg)
...
METHOD TReceposi:Resetar()
	self:New()
return self

METHOD TReceposi:CloneVarColor()
**********************
...
METHOD TReceposi:Reset()
**************
	::RenewVar()
	::ColorToAmbiente()
return self

METHOD TReceposi:ColorToAmbiente()
************************
...
METHOD TReceposi:AmbienteToColor()
************************
...
METHOD TReceposi:AddColor( nColor )
*************************
...
METHOD TReceposi:aChoice_(aTodos, aAtivo, cFuncao, lPageCircular)	
...
METHOD TReceposi:Hello 
	? "Hello", self:cWho
	? "Hello", ::cNome
	return self

METHOD TReceposi:ResetCorSelecao 
...
METHOD TReceposi:ResetSelecao 	
	::ZerarSelecao()
	::RedrawSelecao()	
	::Redraw_()
	return self

METHOD TReceposi:ResetAll 
	::ZerarSelecao()
	::RedrawRecibo()	
	::RedrawVencido()	
	::RedrawVencer()	
...
METHOD TReceposi:ZerarRescisao 	
...
METHOD TReceposi:RedrawVencido 
...
METHOD TReceposi:RedrawSelecao 
...
METHOD TReceposi:_SomaPago( nValorTotal, nValorPago )
*********************************************
...
METHOD TReceposi:ImprimeSoma()
********************
	::aBottom := ::BarraSoma()
	return self

METHOD TReceposi:BarraSoma() 	
****************
	LOCAL xLen		    := Len(::xTodos)
	LOCAL nPrincipal   := 0
	LOCAL nAberto	    := 0
...
METHOD TReceposi:AjustaCorFinal(nAtraso, nT) 	
**********************************
	if !(::aReciboImpresso[nT]) 
		if nAtraso < 0 
...
METHOD TReceposi:sTrFormataATodos(nT) 
***************************
...
METHOD TReceposi:AChoice( nTop, nLeft, nBottom, nRight, acItems, xSelect, xUserFunc, nPos, nHiLiteRow, lPageCircular)
***********************************************************************************************************
...
METHOD TReceposi:HitTest( nTop, nLeft, nBottom, nRight, mRow, mCol )
**********************************************************
	if mCol >= nLeft .AND. ;
      mCol <= nRight .AND. ;
      mRow >= nTop .AND. ;
...
METHOD TReceposi:DispPage( acItems, alSelect, nTop, nLeft, nRight, nNumRows, nPos, nAtTop, nArrLen, nRowsClr )
****************************************************************************************************
   LOCAL nCntr
...
METHOD TReceposi:Displine( cLine, nRow, nCol, lSelect, lHiLite, nNumCols, nCurElemento )
******************************************************************************
...
METHOD TReceposi:Ach_Limits( /* @ */ nFrstItem, /* @ */ nLastItem, /* @ */ nItems, alSelect, acItems)
*******************************************************************************************
   LOCAL nCntr
...
METHOD TReceposi:Ach_Select( alSelect, nPos )
************************************
   LOCAL sel
...
METHOD TReceposi:LeftEqI( string, cKey )	
******************************
	LOCAL nLen := Len( cKey )
