...
Function Main( cAddress, cUser, cPasswd )
   ...
   LOCAL aIni := RdIni( cDirBase + "letobackup.ini" ), ai
   LOCAL nSecs := 30
   LOCAL lWait := .t.
   LOCAL cArc
   LOCAL lSuccess := .f.
   ...
   if lLocked
      ? "Unlocking server ..."
      leto_LockLock( .f. )
      ? if( leto_LockLock( ), "Failure", "Success" )
   endif
   ? "Backup finished"
...
Static function MakeCopy( cAddress, cDB, aTables, cDir, cOutDir, af, nFiles, nTables, lCopyTo )
   ...
   if Empty(ad) .or. Abs(ad[1, 2] - af[2]) > 2 .or. ad[1, 3] != af[3] .or. ad[1, 4] != af[4]
      ...
      if ASCAN(aTables, {|a| Lower(a[2]) == Lower(af[1])}) != 0
         ...
         hb_rddInfo( RDDI_MEMOTYPE, dbInfo( DBI_MEMOTYPE ), "DBFCDX" )
         hb_rddInfo( RDDI_MEMOEXT, dbInfo( DBI_MEMOEXT ), "DBFCDX" )
         ...
         if lCopyTo
            ...
            dbCreate( cNewFile, dbStruct(), "DBFCDX", .t., "dbf_new")
            select leto_old
