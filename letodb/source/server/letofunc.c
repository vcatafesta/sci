...
AREAP leto_RequestArea( ULONG ulAreaID )  // mt
{
   ...
   if( ! pArea )
   {
      ...
      if( ! pArea )
      {
         leto_wUsLog(NULL,"ERROR!!!!!!!!!!! hb_rddRequestArea! 1\r\n", 0);
         return NULL;
      }
   }
...
BOOL leto_DetachArea( ULONG ulAreaID )  // mt
{
   ...
   if( ulAreaID )
   {
      ...
      {
         char ddd[100+HB_RDD_MAX_ALIAS_LEN];
         hb_snprintf( ddd, 100+HB_RDD_MAX_ALIAS_LEN, "ERROR! leto_DetachArea! hb_rddGetAliasNumber! %s\r\n", szAlias );
   ...
   if( pArea )
   {
      ...
      if( !bRetVal )
      {
         ...
         {
            char ddd[100+HB_RDD_MAX_ALIAS_LEN];
            hb_snprintf( ddd, 100+HB_RDD_MAX_ALIAS_LEN, "ERROR! leto_DetachArea! hb_rddDetachArea! %lu", ulAreaID );
            leto_wUsLog( NULL,ddd, 0 );
         }
         else
...
void leto_Ordfunc( PUSERSTRU pUStru, const char* szData )  // mt
{
   ...
   char * szData2 = NULL;
   ULONG ulLen = 4;
   const char * pTagName, * pNumber, * pDopFlags, * pFlags;
   int nParam;
