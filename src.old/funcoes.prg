...
WHILE OK
	...
	Case nChoice = 2
		...
		MaBox( 15, 10, 17 , 79 )
		@ 16, 11 Say "Arquivo de Impressao... Ø " Get oAmbiente:cArquivo Pict "@!"
		Read
		IF LastKey() = 27
			oAmbiente:cArquivo := ""
			ResTela( cScreen )
	...
	Case nChoice = 3
		MaBox( 15, 10, 17 , 79 )
		@ 16, 11 Say "Arquivo a Visualizar... Ø " Get oAmbiente:cArquivo Pict "@!"
		Read
		IF LastKey() = 27
			ResTela( cScreen )
			Exit
...
LOCAL aMensagem		:= Array(3,5)
		...
		aMensagem[3,4] := "    o mesmo Ç de seu conhecimento.                 "
		aMensagem[3,5] := ""
...
IF !oAmbiente:Visual
	...
	*/
	...
	WriteBox( nMaxRow-7, 10, "Esta Ç uma licenáa de uso individual e  intransfer°vel" )
	WriteBox( nMaxRow-6, 10, "para o usuario acima. C¢pia ilegais e nÑo  autorizadas" )
	WriteBox( nMaxRow-5, 10, "Ç crime de PIRATARIA o qual serÑo processadas a m†xima" )
	WriteBox( nMaxRow-4, 10, "extànsÑo da LEI.")
	SetColor("R")
	Write( nMaxRow-2,00, "TECLE ALGO PARA INICIARÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕ")
	For nY := 1 To 1000
		Inkey(.1)
		IF Lastkey() <> 0
			Exit
...
Set Key -9 To
			 WHILE OK
				 oMenu:Limpa()
				 StatusInf("ESCOLHA COM SETAS E TECLE ƒƒƒƒŸ","ESC-RETORNA")
		  M_Title( "OPCOES" )
...
oMenu:Limpa()
ErrorBeep()
nChoice := Alert(" INSTRUÄéO PARA EMISSéO DE ETIQUETAS      " + ;
