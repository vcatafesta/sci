...
MaBox( 14 , 10 , 16 , 53 )
@ 15 , 11 Say	"Arquivo Carta de Cobrança.:" Get Arquivo Pict "@!"
Read
if LastKey() = ESC
	ResTela( cScreen )
	return
...
WHILE !Eof() .AND. Rel_Ok() .AND. !Eof()
	...
	Write( Prow(),   48, Esta )
	Write( Prow()+1, 00, "CIC")
	Write( Prow(),   48, "Carteira de Identidade Nº")
	Write( Prow()+1, 00, Cpf )
	Write( Prow(),   48, Rg )
...
Case oCol:Heading = "CLIENTE"
   ErrorBeep()
	Alerta("Opa! Alteracao não permitida. Altere o cadastro do cliente..")
	return( FALSO )
...
**********************
   ...
   SetColor("")
   Cls
   Grafico( aMes, true,"EVOLUCAO MENSAL DE TITULOS RECEBIDO - &cUltimo. ATÉ &cAno.", cValor, AllTrim(oAmbiente:xNomefir), nBase )
   Inkey(0)
   oAmbiente:cTipoRecibo := NIL
   return(ResTela(cScreen))
endef   
...
*-----------------------------*
	...
	case oCol:Heading == "CLIENTE"
		ErrorBeep()
		MsgOk( oCol:Heading + ";;Opa! Alteracao não permitida!;Altere o cadastro do cliente.")		
		return( FALSO )
