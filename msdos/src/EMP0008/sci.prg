...
PUBL oIni		 := TIniNew( oAmbiente:xRoot + "\SCI.INI")
...
def empresa( cTela )
	...
	WHILE OK
		...
		if Conf('Pergunta: Selecao de Empresa Correta ?')					
			...
			cCmd		  := cBase + _SEP_ + 'CMD'
			cDoc		  := cBase + _SEP_ + 'DOC'		
			cSpooler   := cBase + _SEP_ + 'SPOOLER'
			cTxt       := cBase + _SEP_ + 'TXT'
			cHtm       := cBase + _SEP_ + 'HTM'
			cBaseDados := cBase + _SEP_ + 'EMP' + cCodi
			...
			aIss		  := { Empresa->Iss	}
			FechaTudo()
			ms_makeDir(cBaseDados, cCmd, cDoc, cSpooler, cTxt, cHtm)						
...
def ms_makeDir(cBaseDados, cCmd, cDoc, cSpooler, cTxt, cHtm)
	if (oAmbiente:LetoAtivo)
...
def ms_swap_dbcreate(cArquivo, cFile)
	if (oAmbiente:LetoAtivo)	
		return DbCreate(cArquivo, cFile)		
	endif
	return DbCreate( cArquivo, cFile)		
endef
...
def ms_swap_file(cFile)
	if (oAmbiente:LetoAtivo)	
		return Leto_File(oAmbiente:letopath + cFile)
	endif
	return File(cFile)
endef
...
def ms_swap_ferase(cFile)   	
	...
	if (oAmbiente:Letoativo)
		return leto_ferase(oAmbiente:letopath + cFile)		
	else
		return Ferase(cFile)
	endif	
...
def ms_swap_rename(cFile, cNewFile)   	
	...
	if (oAmbiente:Letoativo)
		return (leto_frename(oAmbiente:LetoPath + cFile, oAmbiente:LetoPath + cNewFile) == 0)		
	else
...
def ms_swap_fopen(cFile, mode)   	
	...
	if (oAmbiente:Letoativo)
		handle := leto_fopen(oAmbiente:letopath + cFile, Mode)		
		return handle
	else
		return Fopen(cFile, Mode)
	endif	
endef
...
def ms_swap_fcreate(cFile, mode)   	
	...
	if (oAmbiente:Letoativo)
		return leto_fcreate(oAmbiente:letopath + cFile, mode)		
	else
		return Fcreate(cFile, mode)
	endif	
endef
...
def ms_swap_fseek(handle, n, mode)   	
	
	if (oAmbiente:Letoativo)
		return leto_fseek(handle, n, mode)
	else
		return fseek(handle, n, mode)
	endif	
endef
...
def SetaIniLeto()
*----------------* 
	//LOCAL oIni		  := TIniNew( oAmbiente:xRoot + "\SCI.INI")
	...
	if oAmbiente:argc	== 0	
		...
		if leto_Connect(cPath) != F_ERROR
			...
			oAmbiente:ShowVarLeto()
			ErrorBeep()
		   if conf("Configuracao Servidor LETO valida em SCI.INI. Desejar conectar?")
