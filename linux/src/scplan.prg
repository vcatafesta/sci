/*
 ...
 ▌│																								 │
 ▌│	Programa.....: SCPLAN.PRG															 │
 ▌│	Aplicacaoo...: SISTEMA DE CONTROLE DE PRODUCAO								 │
 ▌│	Versao.......: 3.3.00																 │
...
oMenu:StatusInf := "F1-HELP║F10-CALC║F8-SPOOL║ESC-RETORNA║"
Return
...
Set Rela To CodiSer Into Servico, Codigo Into Lista
IF !( DbSeek( cTabela ) )
	Cortes->(Escolhe( 00, 00, 24, "Tabela + '▌' + Tran( Sobra, '99999') + '▌' +Left( Lista->Descricao, 30) + '▌' + Left( Servico->Nome,30)", "TABELA  SOBRA  PRODUTO                       SERVICO" ))
...
IF Vendedor->(!DbSeek( cCodi ))
   ...
   Vendedor->(Escolhe( 00, 00, 24, "Codiven + '▌' + Nome", "ID   NOME FUNCIONARIO ", aRotinaInc, NIL, aRotinaAlt ))
   ...
   cCodiSer = Servico->(StrZero( Val( CodiSer ) + 1, 3))
	WHILE OK
      ...
      @ 08, 03 Say "Codigo....:" Get cCodiSer   Pict "999" Valid CodiSer( @cCodiSer )
		@ 09, 03 Say "Serviço...:" Get cNomeSer   Pict "@K!" Valid NomeSer( cNomeSer )
...
IF Empty( cCodi1 )
	ErrorBeep()
	Alerta( "Erro: Codigo Serviço Invalido...")
	Return( FALSO )
EndIf
IF cCodi1 == cCodi2
	DbGoto( Reg_Ant )
...
IF ( DbSeek( cCodi1 ) )
	ErrorBeep()
	Alerta( "Erro: Codigo Serviço Ja Registrado...")
	Return( FALSO )
EndIf
DbGoTo( Reg_Ant )
Return(OK)
...
IF Servico->(!DbSeek( cServico ))
	Servico->(Escolhe( 00, 00, 24, "Nome", "         SERVIÇO", aRotina ))
...
IF ( Empty( cNome ) )
	ErrorBeep()
	Alerta("Erro: Nome Serviço Invalido..." )
	Return( FALSO )
EndIF
Area( "Servico" )
Servico->(Order( SERVICO_NOME ))
IF ( DbSeek( cNome ) )
	ErrorBeep()
	Alerta("Erro: Nome Serviço Ja Registrado..." )
	AreaAnt( Arq_Ant, Ind_Ant )
	Return( FALSO )
EndIf
AreaAnt( Arq_Ant, Ind_Ant )
...
WHILE OK
    ...
    Do Case
	 ...
	 Case nChoice = 2
		 ...
		 @ 19, 21 Say "Docto Nº...:" Get cDocnr Pict "@!" Valid DocFuErrado( @cDocnr )
		 Read
		 IF LastKey() = ESC
			 Loop
		 EndIF
...
Mostra1 := {"CODI", "NOME", "DEBITO", "CREDITO" ,"DATA" ,"DOCTO Nº" ,"DESCRICAO"}
...
WHILE OK
   ...
   @ 10, 11 Say "Codigo....:" Get cCodi Pict "9999" Valid ProduErrado( @cCodi,NIL, Row(), Col()+1 )
	...
	@ 13, 11 Say "Docto Nº..:" Get cDocnr    Pict "@!"
...
IF !( DbSeek( cDocnr ))
	Escolhe( 00, 00, 24, "Docnr", "  DOCTO Nº  " )
	cDocnr := Docnr
EndIf
Return( OK )
...
While !Eof() .AND. REL_OK()
   IF Col >=  58
      ...
      Write( 05, 00, Repl( SEP, Tam ) )
      Write( 06, 00,"DATA     DOCTO Nº  DESCRICAO                                        DEBITO        CREDITO")
      Write( 07, 00, Repl( SEP, Tam ) )
      Col := 8
   EndIf
   IF Col = 8
