...
PUBL aInscMun	 := {}
PUBL aIss		 := {}
PUBL XCFGPIRACY := MsCriptar( ENCRYPT )
PUBL oAmbiente  := TAmbienteNew()
PUBL oMenu		 := TMenuNew()
...
EndIF
//SetaIni()
//oMenu:Panos[1] := CapFirst( XNOMEFIR ) + " "
oMenu:Limpa()
SetaClasse()
RefreshClasse()
WHILE lOk
...
Aadd( aArquivos, { "RECEMOV",   "RECEMOV1","RECEMOV2","RECEMOV3","RECEMOV4","RECEMOV5","RECEMOV6","RECEMOV7", "RECEMOV8"})
...
Area("Empresa")
IF !DbSeek( cCodi )
   Escolhe( 00, 00, MaxRow(), "Codi + ' ' + Nome", "CODIGO     NOME DA EMPRESA", aRotina )
EndIF
cCodi := Empresa->Codi
cNome := Empresa->Nome
IF nRow != Nil
...
LOCAL cSenha1 := StrTran(Dtoc(Date()),'/')
LOCAL Passe   := Criptar(Alltrim(Upper(cPassword)))
...
IF !Empty( Passe) .AND. cSenha == Passe
   ...
   Aadd( aPermissao, IF( Usuario->(DCriptar( NivelO)) = "S", OK, FALSO ))
   Aadd( aPermissao, IF( Usuario->(DCriptar( NivelP)) = "S", OK, FALSO ))
   Aadd( aPermissao, IF( Usuario->(DCriptar( NivelQ)) = "S", OK, FALSO ))
   Aadd( aPermissao, IF( Usuario->(DCriptar( NivelR)) = "S", OK, FALSO ))
   Aadd( aPermissao, IF( Usuario->(DCriptar( NivelS)) = "S", OK, FALSO ))
...
IF Usuario->(!DbSeek( cNome ))
   Escolhe( 00, 00, MaxRow(), "Nome", "USUARIO" )
	cNome := Usuario->Nome
	Return( OK )
EndIF
Return( OK )
...
IF !lDireto
   ...
   IF lAdmin
      cSenha1 := DCriptar( AllTrim( cSenha ))
      lOk     := OK
      ResTela( cScreen )
      Return
   EndIF
...
WHILE OK
   IF !lDireto
      ...
      EndIF
   EndIF
   IF cSenha == Criptar( AllTrim( cSenha1 )) .OR. lDireto
		WHILE OK
			Write( 02, 11, "Nova Senha...........: " )
			Write( 03, 11, "Confirme a Senha.....: " )
			cSenha1 := Senha( 02, 34, 11 )
...
WHILE OK
	...
	Case nChoice = 2
		...
		cFat		 := DCriptar( NivelQ )
		cDtRec	 := DCriptar( NivelR )
		cAtra 	 := DCriptar( NivelS )
		cLpt1 	 := Usuario->Lpt1
		cLpt2 	 := Usuario->Lpt2
		cLpt3 	 := Usuario->Lpt2
	...
	IF CadPermissao( cNome, @cEs, @cRe, @cPa, @cCo, @cVe, @cVn, @cUs, @cPr, @cInc, @cDel, @cDev, @cPag, @cRec, @cAlt, @cEmp, @cTro, @cFaz, @cRes, @cCom, @cZero, @cVista, @cCadas, @cLimite, @cEstorado, @cLpt1, @cLpt2, @cLpt3, @cDescMax, @cEnt, @cFat, @cDtRec, @cAtra )
		...
		IF Conf( "Pergunta: Incluir/Alterar Usuario ?")
			...
			Usuario->NivelQ := Criptar( cFat 	 )  //  27
			Usuario->NivelR := Criptar( cDtRec	 )  //  28
			Usuario->NivelS := Criptar( cAtra	 )  //  29
			Usuario->Lpt1	 := cLpt1
			Usuario->Lpt2	 := cLpt2
			Usuario->Lpt3	 := cLpt3
			Usuario->(Libera())
...
WHILE OK
   ...
   */
   ...
   nSoma        := 0
   nQtDocumento := 0
   Mensagem("Aguarde, Imprimindo Relatorio de Recibos.", Cor())
   PrintOn()
   Fprint( Chr(ESC) + "C" + Chr(33))
   FPrint( PQ )
   SetPrc(0,0)
   CabecCaixa( ++Pagina, Tam, cTitulo, cCaixa, cTitular )
   Write( 06, 00,"TIPO    CODI NOME CLIENTE                     DOCTO N§     VCTO    PAGTO     HORA RECEBIDO CAIXA HISTORICO")
   ...
   WHILE Recibo->(!Eof()) .AND. Rel_Ok()
      ...
      IF cTipoDetalhe == 1
         IF Recibo->Tipo == "RECBCO" .OR. Recibo->Tipo == "RECBCO"
            Recibo->(DbSkip(1))
            Loop
         EndIF
      EndIF
      ...
      /*
      IF Recibo->Tipo == "DIARIA"
         nVlr := -nVlr
      EndIF
      */

      Recibo->( Qout( Tipo, Codi, Left(Nome,31), Docnr, Vcto, Data, Hora, Tran( nVlr, "@E 9,999.99"), Left(Usuario,5), Left(Hist,35)))
      nQtDocumento++
      nSoma += nVlr
      Col++
      Recibo->(DbSkip(1))
   EndDo
   Qout()
   Qout("TOTAL REGISTROS..:", StrZero( nQtDocumento, 4) + Space(59) + NG + AllTrim(Tran( nSoma, "@E 999,999.99")) + NR)
   Qout()
   Qout()
   Qout(Repl("_",40))
...
IF lAtraso
	...
	WHILE OK
		...
		nChoice := FazMenu( 15, 10, aMenu, 75 )
		Do Case
...
WHILE OK
   ...
   oMenu:MaBox( 01, 01, 18, 78, "CONFIGURACAO - GERAL")
...
EndIF
AADD( AtPrompt, {"E^ncerrar",    {"E^ncerrar Execucao do SCI","","T^rocar de Empresa","Trocar de Usuario","","Copia de Seguranca","Restaurar Copia de Seguranca"}})
...
AADD( AtPrompt, {"A^mbiente",    {"Spooler de Impressao","Fontes de Letras","", "Cor Pano de Fundo","Cor de Menu","Cor Cabecalho","Cor Alerta","Cor Borda","Cor Item Desativado","Cor Box Mensagem", "","Pano de Fundo","Frame", cStr_Sombra, cStr_Get, "Alterar Senha","Cor Light Bar", "Cor HotKey", "Cor LightBar HotKey"}})
