/*
	...
	#define NAT_STR  "ACELNŸSZZaceln¢szz"
	#define NAT_CP   { "PTISO", "PLISO", "PLMAZ", "PLWIN", "PL852" }
	...
	PROCEDURE Main( cTermCP, cHostCP, lBoxChar )
	   ...
	   cTermCP := Upper( hb_defaultValue( cTermCP, "PTISO" ) )
	   ...
	   NEXT
	   Inkey( 0 )
	   ?; dspboxch( hb_UTF8ToStrBox( "ÚÄÂÄ¿  ÉÍËÍ»  ÕÍÑÍ¸  ÖÄÒÄ·  ÜÜÜ °±²Û  ° ± ² Û" ) )
	   ?; dspboxch( hb_UTF8ToStrBox( "³ ³ ³  º º º  ÃÄÅÄ´  ÇÄ×Ä¶  ÝþÞ" ) )
	   ?; dspboxch( hb_UTF8ToStrBox( "ÃÄÅÄ´  ÌÍÎÍ¹  ³ ³ ³  º º º  ÝÛÞ ÛþÛ" ) )
	   ?; dspboxch( hb_UTF8ToStrBox( "³ ³ ³  º º º  ÆÍØÍµ  ÌÍÎÍ¹  ÝþÞ" ) )
	   ?; dspboxch( hb_UTF8ToStrBox( "ÀÄÁÄÙ  ÈÍÊÍ¼  ÔÍÏÍ¾  ÓÄÐÄ½  ßßß û ® ¯" ) )
	   ?
